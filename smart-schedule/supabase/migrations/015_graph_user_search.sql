-- Enable pgsql-http extension for outbound HTTP requests
CREATE EXTENSION IF NOT EXISTS http;

-- Search Entra ID directory users via Microsoft Graph API.
-- Uses client credentials flow (app-only) so it works without a user token.
-- Returns a JSON array of matching users.
...
-- Grant execute to authenticated role (PostgREST)
